class Solution {
    ...
    private int[][] convexHull(int[][] trees) {
        ...
        for (int i = 0; i < trees.length; i++) {
            while (k >= 2 && this.cross(lowerAndUpper[k - 2], lowerAndUpper[k - 1], trees[i]) > 0) k--;
            lowerAndUpper[k++] = trees[i];
        }
        for (int i = trees.length - 2, t = k + 1; i >= 0; i--) {
            while (k >= t && this.cross(lowerAndUpper[k - 2], lowerAndUpper[k - 1], trees[i]) > 0) k--;
            lowerAndUpper[k++] = trees[i];
        }
    ...
    public int[][] outerTrees(int[][] trees) {
        ...
        for (int i = 0; i < convexArray.length - 1; i++) {
            if (convexArray[i][0] == convexArray[i + 1][0] && convexArray[i][1] == convexArray[i + 1][1]) continue;
